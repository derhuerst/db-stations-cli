...
const cliSize = require('window-size')
...
const createFlatten = (columns) => {
	const flatten = (s) => {
		...
		for (let col of columns) {
			if (col === 'coords') {
				if (s.location) {
					r.latitude = s.location.latitude
					r.longitude = s.location.longitude
				}
			} else r[col] = s[col]
		}
...
const pretty = (columns, stations, out) => {
	...
	return through.obj((s, _, cb) => {
		...
		const line = truncate(parts.join(' '), cliSize ? cliSize.width : 80) + '\n'
		cb(null, line)
	})
}
